 ...
 *
 * Original author: Mustafa Ekşi <mustafa.eskieksi@gmail.com>
 * Upstream: https://github.com/Mustafa-eksi/casper-wmi
 *
 * Fork changes (v1.1.0), by the excalibur-center project:
 *   - Fix NULL pointer dereference in casper_wmi_hwmon_read() on models
 *     that are not listed in casper_dmi_list (e.g. EXCALIBUR G770).
 *     The fan read path dereferenced a static pointer that was only set
 *     by dmi_matched(); on unmatched systems any read of fan*_input
 *     crashed the kernel task reading it.
 *   - Add EXCALIBUR G770 and a generic CASPER fallback to the DMI table.
 *   - Declare HWMON_PWM_INPUT so that the power plan is actually
 *     exposed as pwm1 in sysfs (the write handler existed but was
 *     unreachable because only HWMON_PWM_MODE was declared).
 *
...
/*
 * Fork fix: default to &has_raw_fanspeed instead of NULL so that models
 * missing from casper_dmi_list never dereference a NULL pointer.
 */
static bool *casper_raw_fanspeed = &has_raw_fanspeed;

static int dmi_matched(const struct dmi_system_id *dmi)
{
	pr_info("casper-wmi: identified laptop model '%s'\n", dmi->ident);
...
static const struct dmi_system_id casper_dmi_list[] = {
	...
	},
	{
	 ...
	 /* Verified on hardware: EC reports big-endian fan speed. */
	 .driver_data = &no_raw_fanspeed,
	},
	{
	 .callback = dmi_matched,
...
static const struct hwmon_channel_info *const casper_wmi_hwmon_info[] = {
	HWMON_CHANNEL_INFO(fan,
			   ...
			   HWMON_F_INPUT | HWMON_F_LABEL),
	/*
	 * Fork fix: declare HWMON_PWM_INPUT alongside HWMON_PWM_MODE so the
	 * power plan becomes writable/readable as pwm1 in sysfs. Upstream only
	 * declared the mode attribute, leaving the read/write handlers dead code.
	 */
...
static int casper_wmi_probe(struct wmi_device *wdev, const void *context)
{
	...
	if (!dmi_check_system(casper_dmi_list)) {
		// Unmatched model: safe defaults already applied via
		// casper_raw_fanspeed = &has_raw_fanspeed above.
		dev_info(&wdev->dev,
			 "Unrecognized Casper model; assuming raw fan speed values. If fan speeds look wrong, report to excalibur-center.");
	}
